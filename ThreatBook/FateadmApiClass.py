...
class FateadmApi():
    ...
    def QueryBalc(self):
        ...
        if rsp.status_code == 0:
            LOG("query succ ret: {}  pred: {}".format(rsp.status_code, rsp.text))
        else:
            LOG("query failed ret: {} err: {}".format(rsp.status_code, rsp.text.encode('utf-8')))
        return rsp
    ...
    def Predict(self, pred_type, img_data):
        ...
        files = img_data
        r = PostFile(url, param, files)
        rsp = json.loads(r)
        # print(rsp)
        if rsp.get('RetCode') == '0':
            pass
            # LOG("predict succ ret: {} request_id: {} pred: {} err: {}".format(rsp.RetCode, rsp.RequestId,
            #                                                                   rsp.RspData.result, rsp.ErrMsg))
        else:
            pass
            # LOG("predict failed ret: {} err: {}".format(rsp.RetCode, rsp.ErrMsg.encode('utf-8')))
            if rsp.get('RetCode') == 4003:
                # lack of money
...
def CalcSign(pd_id, passwd, timestamp):
    md5 = hashlib.md5()
    md5.update(timestamp.encode("utf-8") + passwd.encode("utf-8"))
    csign = md5.hexdigest()

    md5 = hashlib.md5()
    md5.update(pd_id.encode("utf-8") + timestamp.encode("utf-8") + csign.encode("utf-8"))
    csign = md5.hexdigest()
    return csign


def HttpRequest(url, body_data):
    rsp = requests.post(url, body_data, headers={"User-Agent": "Mozilla/5.0"})
    return rsp
